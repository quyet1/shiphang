CREATE DATABASE IF NOT EXISTS SHIPCOM CHARACTER SET 'utf8' COLLATE 'utf8_unicode_ci';
USE SHIPCOM;

DROP TABLE IF EXISTS DANHMUCTIN;
CREATE TABLE IF NOT EXISTS DANHMUCTIN(
    MADANHMUCTIN VARCHAR(25) PRIMARY KEY NOT NULL,
    TENDANHMUCTIN VARCHAR(100),
    MOTA VARCHAR(500)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;

INSERT INTO DANHMUCTIN(MADANHMUCTIN, TENDANHMUCTIN , MOTA) VALUES
('DM001', 'Shipper', 'Nhận, giao hàng'),
('DM002', 'Làm Việc Nhanh', 'Làm việc nhanh theo yêu cầu của người khác'),
('DM003', 'Đi Chung', 'Cho người khác đi chung xe hoặc đi xe chung với người khác');

DROP TABLE IF EXISTS QUYENQUANTRI;
CREATE TABLE IF NOT EXISTS QUYENQUANTRI(
    MAQUYEN VARCHAR(25) PRIMARY KEY NOT NULL,
    TENQUYEN VARCHAR(50),
    MOTA VARCHAR(100)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS THANHVIEN(
    MATHANHVIEN VARCHAR(25) PRIMARY KEY NOT NULL,
    TENTHANHVIEN VARCHAR(100),
    NGAYSINH DATE,
    DIACHI VARCHAR(255),
    EMAIL VARCHAR(50),
    SODIENTHOAI VARCHAR(25), -- Bắt buộc có
    SOCMND VARCHAR(15),
    HINHDAIDIEN VARCHAR(100),-- Đường dẫn file ảnh
    HINHCMNDT VARCHAR(50),
    HINHCMNDS VARCHAR(50),
...
CREATE TABLE IF NOT EXISTS PHANHOI(
    MAPH MEDIUMINT PRIMARY KEY AUTO_INCREMENT,
    TIEUDE VARCHAR(50),
    NOIDUNG TEXT,
    NGAYPH DATETIME DEFAULT CURRENT_TIMESTAMP,
    MATHANHVIEN VARCHAR(25),
    FOREIGN KEY KF_PHANHOI_THANHVIEN (MATHANHVIEN) REFERENCES THANHVIEN(MATHANHVIEN)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;

DROP TABLE IF EXISTS QUANTRI;
CREATE TABLE IF NOT EXISTS QUANTRI(
    MAQT VARCHAR(25) PRIMARY KEY NOT NULL,
    TENQT VARCHAR(150),
    NGAYSINH DATE,
    DIACHI VARCHAR(150),
    EMAIL VARCHAR(50),
    ...
    MAQUYEN VARCHAR(25),
    FOREIGN KEY FK_QUYENQUANTRI_MAQUYEN (MAQUYEN) REFERENCES QUYENQUANTRI(MAQUYEN)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;

DROP TABLE IF EXISTS TRALOIPHANHOI;
CREATE TABLE IF NOT EXISTS TRALOIPHANHOI(
    MATRALOIPH MEDIUMINT PRIMARY KEY AUTO_INCREMENT,
    NGAYTRALOIPH DATETIME DEFAULT CURRENT_TIMESTAMP,
    NOIDUNG TEXT,
    MAQT VARCHAR(25),
    MAPH MEDIUMINT,
    FOREIGN KEY KF_TRALOIPH_PHANHOI (MAPH) REFERENCES PHANHOI(MAPH),
    FOREIGN KEY KF_TRALOIPH_QUANTRI (MAQT) REFERENCES QUANTRI(MAQT)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;

DROP TABLE IF EXISTS TAIKHOAN;
CREATE TABLE IF NOT EXISTS TAIKHOAN(
    MATAIKHOAN VARCHAR(25) PRIMARY KEY NOT NULL,
...
CREATE TABLE IF NOT EXISTS DISTRICT(
    ...
    FOREIGN KEY FK_DISTRICT_PROVINCE_PROVINCEID (PROVINCEID) REFERENCES PROVINCE(PROVINCEID)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

DROP TABLE IF EXISTS TINDANG;
CREATE TABLE IF NOT EXISTS TINDANG(
    MATIN BIGINT PRIMARY KEY AUTO_INCREMENT,
    TIEUDE VARCHAR(255),
    NOIDUNG TEXT, -- Tương đương với mô tả
    DIADIEMDAU VARCHAR(5),
    DIADIEMCUOI VARCHAR(5),
    NGAYDANG DATETIME DEFAULT CURRENT_TIMESTAMP,
    GIA DECIMAL,
    KHOILUONGHANG FLOAT,
    MAQT VARCHAR(25), -- =? Xác định tin đã được duyệt bởi MAQT : NULL 
    MATHANHVIEN VARCHAR(25),
    MADANHMUCTIN VARCHAR(25),
    TINHTRANG VARCHAR(25),-- Xác định người thực hiện giao dịch tin, chứa mã thành viên - hoặc invalid --
    
    FOREIGN KEY FK_TIN_THANHVIEN_MATHANHVIEN (MATHANHVIEN) REFERENCES THANHVIEN(MATHANHVIEN),
    FOREIGN KEY FK_TIN_DANHMUCTIN (MADANHMUCTIN) REFERENCES DANHMUCTIN(MADANHMUCTIN),
    FOREIGN KEY FK_TIN_DISTRICTd (DIADIEMDAU) REFERENCES DISTRICT(DISTRICTID),
    FOREIGN KEY FK_TIN_DISTRICTc (DIADIEMCUOI) REFERENCES DISTRICT(DISTRICTID)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
DROP TABLE IF EXISTS IMAGES;
CREATE TABLE IF NOT EXISTS IMAGES(
    MAHINH MEDIUMINT PRIMARY KEY AUTO_INCREMENT,
    TENHINH VARCHAR(25),
    DUONGDAN VARCHAR(50),
    MATIN BIGINT,
    FOREIGN KEY FK_IMAGES_TINDANG_MATIN (MATIN) REFERENCES TINDANG(MATIN)
)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
INSERT INTO PROVINCE(PROVINCEID, FULLNAME , PTYPE) VALUES
('01', 'Hà Nội', 'Thành Phố'),
('02', 'Hà Giang', 'Tỉnh'),
('04', 'Cao Bằng', 'Tỉnh'),
('06', 'Bắc Kạn', 'Tỉnh'),
...
INSERT INTO DISTRICT(DISTRICTID, FULLNAME, DTYPE, LOCATION, PROVINCEID) VALUES
